...
#[AsCommand(
    name: 'app:items:download-images',
    description: 'Download item images from DataDragon CDN to local storage',
)]
final class DownloadItemImagesConsoleCommand extends Command
{
    public function __construct(
        private readonly ItemAssetDownloaderInterface $downloader,
    ) {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->info(sprintf('Fetching item list for version %s...', $version));
        ...
        try {
            ...
            $io->newLine(2);
            $io->success(sprintf(
                '%d image(s) downloaded to public/images/items/%s/ (%d already present)',
                $downloaded,
                $version,
